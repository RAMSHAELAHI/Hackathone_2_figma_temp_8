        ...
        <div className="container mx-auto flex justify-between items-center px-6">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1">
              <span>English</span>
              <RiArrowDropDownLine className="text-3xl" aria-label="Change Language" />
            </div>
            <div className="flex items-center gap-2">
              <Link href="/faq" className="flex items-center gap-2 text-white hover:text-gray-300">
                FAQ's
                <IoIosHelpCircleOutline className="text-xl" aria-label="FAQ Help" />
              </Link>
              <Link href="/support" className="hover:text-gray-300">Need Help</Link>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="container mx-auto flex justify-between items-center px-6">
          ...
          <nav className="flex items-center gap-8 text-sm font-medium">
            {navLinks.map(({ href, label }) => (
              <Link key={href} href={href} className="text-gray-600 hover:text-gray-800">
                {label}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-6">
            {cartItems.map(({ href, label, icon, count }) => (
              <div key={href} className="flex items-center gap-2">
                {icon}
                <Link href={href} className="text-gray-800 font-medium">{label}</Link>
                <span className="bg-pink-500 text-white text-xs font-bold px-2 py-1 rounded-full">{count}</span>
              </div>
            ))}
            {/* Contact */}
            <div>
              <a href="tel:03238277117" className="text-gray-600 hover:text-gray-800 font-medium">
